  ...
  private parseInstances(params: CliParams): InstanceConfig[] {
    if (params.instances) {
      ...
      for (const entry of entries) {
        ...
        // Handle URLs with protocol (https://host → parts[1] + ':' + parts[2])
        if (parts[1]?.startsWith('//') || parts[2]?.startsWith('//')) {
          // "name:https://host:user:pass" splits as [name, https, //host, user, pass]
          name = parts[0];
          host = parts[1] + ':' + parts[2];
          user = parts[3];
          pass = parts.slice(4).join(':'); // password may contain colons
        } else {
          name = parts[0];
          host = parts[1];
          user = parts[2];
    ...
    // Single instance from CLI flags (backward compatible)
    return [{
      name: 'default',
